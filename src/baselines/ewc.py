...
def get_fisher(model, to_train_data, training_config):
    softmax_func = torch.nn.LogSoftmax(dim=-1)
    loss_func = torch.nn.NLLLoss()
    fisher_batch_size = 1
    to_train_data = shuffle(to_train_data)
    ...
    training_text, training_pos, training_neg = wrapped_to_train_data
    batch_epoch = (len(to_train_data) - 1) // fisher_batch_size + 1
    fisher = None
    model.train()
    ...
    for i in range(batch_epoch):
        ...
        length_list = [
            len(train_tuple[2])
            for train_tuple in to_train_data[
                i * fisher_batch_size : (i + 1) * fisher_batch_size
            ]
        ]
        num_samples = sum(length_list)
        samples_text = torch.tensor(
            training_text[start_idx : start_idx + num_samples],
            dtype=torch.long,
        )
        if torch.cuda.is_available():
            samples_text = samples_text.to(gpu)
        samples_pos = torch.tensor(
            training_pos[start_idx : start_idx + num_samples], dtype=torch.long
        )
        if torch.cuda.is_available():
            samples_pos = samples_pos.to(gpu)
        samples_neg = torch.tensor(
            training_neg[start_idx : start_idx + num_samples], dtype=torch.long
        )
        if torch.cuda.is_available():
        ...
        for length in length_list:
            pos_score = pos_scores[inner_start_idx : inner_start_idx + length]
            neg_score = neg_scores[inner_start_idx : inner_start_idx + length]
            # print(pos_score[-1].size())
            # print(neg_score.size())
            all_score = torch.cat(
                (pos_score[-1].unsqueeze(0), neg_score.view(-1)), dim=-1
            )
            target = torch.tensor([0], dtype=torch.long)
            if torch.cuda.is_available():
                target = target.cuda(training_config["gpu"])
            losses.append(
                loss_func(softmax_func(all_score).view(1, -1), target)
            )
            inner_start_idx += length
        assert inner_start_idx == samples_text.size(0)
        loss_batch = sum(losses)
        loss_batch.backward()
        grad_params = get_grad_params(model)
        if fisher is None:
            fisher = [param.grad**2 / batch_epoch for param in grad_params]
        else:
            fisher = [
                fisher[parami] + param.grad**2 / batch_epoch
                for parami, param in enumerate(grad_params)
            ]
        start_idx += num_samples
    assert start_idx == len(training_text)
    # print(fisher)
    return fisher


def get_mean_fisher(model, to_train_data, training_config):
    grad_params = get_grad_params(model)
    grad_mean = copy_param_data(grad_params)
    grad_fisher = get_fisher(model, to_train_data, training_config)
    return grad_mean, grad_fisher


def param_loss(model, means, fishers, p_lambda, gpu):
    grad_params = get_grad_params(model)
    ...
    for epoch in range(training_config["epoch"]):
        ...
        for i in range((len(training_text) - 1) // batch_size + 1):
            ...
            if torch.cuda.is_available():
                samples_neg = samples_neg.to(gpu)
            optimizer.zero_grad()
            pos_score, neg_socre, loss1 = model(
                samples_text, samples_pos, samples_neg
            )
            avgloss1 += loss1.data.item() * samples_text.size(0)
            loss2 = torch.tensor(0.0)
            if torch.cuda.is_available():
                loss2 = loss2.to(gpu)
            for grad_i in range(len(grads_means)):
                grad_mean = grads_means[grad_i]
                grad_fisher = grads_fishers[grad_i]
                loss2 += param_loss(
                    model,
                    grad_mean,
                    grad_fisher,
                    training_config["p_lambda"],
                    training_config["gpu"],
                )
            avgloss2 += loss2.data.item() * samples_text.size(0)
            loss = loss1 + loss2
            loss.backward()
            optimizer.step()
        ...
        avgloss1 /= len(training_text)
        avgloss2 /= len(training_text)
        ...
        if prev_loss - avgloss < 5e-3 and avgloss < 0.1:
            break
        # if eval_dict['macro_f1']<=prev_dev_macro and eval_dict['micro_f1']<=prev_dev_micro and prev_dev_micro>0.3:
        #    break
        prev_loss = avgloss
        prev_dev_macro = eval_dict["macro_f1"]
        prev_dev_micro = eval_dict["micro_f1"]
...
def run_sequence(
    ...
    for i, fname in enumerate(cat_order):
        ...
        for sample_ind, sample in enumerate(memory_data):
            neg_cands = [cand for cand in seen_groups if cand != sample[1]]
            neg_samples = random.sample(
                neg_cands, min(len(neg_cands), training_config["cand_limit"])
            )
            memory_data[sample_ind] = (sample[0], sample[1], neg_samples)

        to_train_data = current_training_data + memory_data
        ...
        grad_mean, grad_fisher = get_mean_fisher(
            model, to_train_data, training_config
        )
        grads_means.append(grad_mean)
        grads_fishers.append(grad_fisher)

        sample_size = int(training_config["memory_size"] / (i + 1))
        if training_config["memory_replay"] == "RAND":
            memory_data = random_update_memory(
                memory_data,
                current_training_data,
                training_config["memory_size"],
                sample_size,
            )
...
def main():
    ...
    parser.add_argument(
        "--p_lambda", default=1e6, type=float, help="penalty loss weight"
    )
    parser.add_argument(
        "--similarity",
        default="cos",
